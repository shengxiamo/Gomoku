...
POINTS chessAI::seekPoints(int board[15][15],int flag,int depth)
{
    ...
    for(int i=0;i<15;++i)// 每个非空点附近8个方向延伸3个深度,若不越界则标记为可走
    {
        for(int j=0;j<15;++j)
        {
            if (board[i][j] !=C_NONE&&myChesses[i][j]!=0) // 非空
            {
                for(int k=-3;k<=3;++k)
                {
                    if(i+k>=0&&i+k<15)
...
int chessAI::analyse(int (*board)[15], int depth,int alpha, int beta) // 返回值为最佳走法对应的最终分数
{
    //用来区分算杀，精确值节点和a,b结点
    ...
    if(depth == 0 || EVAL.result != R_DRAW) // 抵达最深层或模拟落子可以分出输赢，那么直接返回结果，不需要再搜索
    {
        ...
        if(depth==0)
        {
            ...
            if (zobb.probe(hashkey,depth,rescore,reflag)) //如果有精确结果就返回精确结果
            {
                if (reflag==EXACT)
                    return rescore;
            }
    ...
    else if (depth % 2 == 0) //max层,我方(白)决策
    {
        ...
        for(int i = 0;i < 10;++i)
        {
            ...
            else
            {
                ...
                // analyse递归调用结束后，应该进行回溯，回到上一层棋局,开始正式进行剪枝搜索
                hashkey=zobb.calculateHash(sameBoard);
                zobb.store(hashkey,depth,a,ALPHA);
            }
            if(a>alpha) // 发现对于max层更优的节点
            {
                alpha=a;// aplha更新
                //必须要在最外层的递归中设置位置
                if(depth == 6) // 记录这次的落子位置
                {
            ...
            if(beta<=alpha)break;// max层节点大于其父节点的beta值，则进行alpha剪枝
        }
        return alpha;
    }
    else //min层,敌方(黑)决策
    {
        ...
        for(int i=0;i<10;++i)
        {
            ...
            if(a<beta)beta=a;// min层更新
            if(beta<=alpha)break;// // min层节点小于其父节点的alpha值，则进行beta剪枝
        }
        return beta;
    }
}
